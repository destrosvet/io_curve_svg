...
def SVGGetMaterial(color, context):
    ...
    if color.startswith('#'):
        ...
        c = re.findall('(?:\d+\.\d+)|(?:\d+,\d+)',color)
        diff = (float(c[0])*2.55, float(c[1])*2.55, float(c[2])*2.55)
    else:
        return None
...
class SVGGeometryContainer(SVGGeometry):
    ...
    def _doCreateGeom(self, instancing):
        ...
        self._geometries[:] = self._geometries[::-1]
...
class SVGLoader(SVGGeometryContainer):
    ...
    def __init__(self, filepath, do_colormanage,global_scale,reverse_order):
        """
        ...
        m = Matrix()
        m = m * Matrix.Scale(global_scale / 90.0 * 0.3048 / 12.0, 4, Vector((1.0, 0.0, 0.0)))
        m = m * Matrix.Scale(-global_scale / 90.0 * 0.3048 / 12.0, 4, Vector((0.0, 1.0, 0.0)))
...
def load_svg(filepath, do_colormanage, global_scale,reverse_order):
    """
    ...
    loader = SVGLoader(filepath, do_colormanage, global_scale,reverse_order)
    loader.parse()
    loader.createGeom(False)


def load(operator, context, filepath="", global_scale=1.0, reverse_order=False):
    ...
    try:
        load_svg(filepath, do_colormanage, global_scale,reverse_order)
